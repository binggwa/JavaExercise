...
Java는 **한 부모에게만 클래스를 상속**하는 매우 독특한 상속 환경,
_인터페이스_를 사용하면 다른 부모에게 있는 것처럼 다르게 보이게 만들 수 있음.
...
부모는 **최소한 메서드의 이름과 signature를 지정**해야 한다.

signature는 파라미터의 유형과 순서에 따라 signature를 인식하기 때문.

+ 메서드의 signature란?

> 메서드 이름과 매개변수 리스트의 조합을 말한다.

> 오버로딩 때문에 메서드의 이름과 매개변수 리스트는 중요하다.
